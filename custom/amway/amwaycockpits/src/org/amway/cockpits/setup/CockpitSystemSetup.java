 ...
 *
 */
package org.amway.cockpits.setup;
...
import org.amway.cockpits.constants.AmwayCockpitsConstants;
 ...
 */
@SystemSetup(extension = AmwayCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(AmwayCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(AmwayCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						AmwayCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/amwaycockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
